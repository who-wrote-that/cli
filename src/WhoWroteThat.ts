...
export default class WhoWroteThat {
    ...
    constructor(filePath: string, depth: number, strategy: Strategy, fail: (error: string) => void) {
    ...
    decl(name: string): Promise<CodeOwners> {
        return commitsAffectingFile(this.filePath).then(commits => {
            return readFile(this.filePath).then(sourceCode => {
                ...
                return this.rec(declaration, commits, 0).then(owners => ({ declaration, owners }));
    ...
    private rec(declaration: Declaration, commits: string[], commitIndex: number): Promise<Owner[]> {
        if (this.depth && commitIndex >= this.depth || commitIndex >= commits.length)
        ...
        return readFileAtCommit(this.filePath, commits[commitIndex])
            .then(sourceCodeAtCommit => {
                ...
                const spans = this.parser.findSpans(node, declaration);

                // if current commit does not contain declaration
                // assume no earlier commit contains declaration
                if (spans.length == 0) return [];

                return Promise.all(spans.map(span => getOwnerOfCommit(this.filePath, commits[commitIndex], span)))
                    .then(mergeDuplicateOwners)
                    .then(owners => {
                        return this.rec(declaration, commits, commitIndex + 1).then(newOwners => {
                            return mergeDuplicateOwners([...owners, ...scale(this.strategy, newOwners, weight(this.strategy, owners))]);
                        });
                    }).catch(error => {
                        console.error(error);
                        return [];
...
const scale = (strategy: Strategy, owners: Owner[], weight: number): Owner[] => {
