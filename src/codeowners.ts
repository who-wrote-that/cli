...
// import {Declaration, findDeclaration, findDeclarationByName, findSpans} from './parse';
...
//   fail = (error: string) => {
//     if (this.format === 'json')
//       console.error(JSON.strignify({ error }))
//     else
//       console.error(error)
//
//     process.exit(1)
...
// export const codeOwnersByLine = (filePath: string, line: number, depth: number, strategy: string): Promise<{def: Declaration; owners: Owner[]}> => {
...
//             return codeOwnersRec(filePath, depth, strategy, def, commitHashes, 0).then(owners => ({def, owners}));
//         }).then(transformResult);
//     });
// };
//
// export const codeOwnersByDefName = (filePath: string, defName: string, depth: number, strategy: string): Promise<{def: Declaration; owners: Owner[]}> => {
//     return commitsAffectingFile(filePath).then(commitHashes => {
//         return readFile(filePath).then(sourceCode => {
//             const suffix = filePath.split('.').pop();
//             const def = findDeclarationByName(suffix, sourceCode, defName);
//             return codeOwnersRec(filePath, depth, strategy, def, commitHashes, 0).then(owners => ({def, owners}));
//         }).then(transformResult);
//     });
// };
//
// const codeOwnersRec = (filePath: string, depth: number, strategy: string, def: Declaration, commitHashes: string[], commitIndex: number): Promise<Owner[]> => {
...
//                 .then(owners => {
//                     return codeOwnersRec(filePath, depth, strategy, def, commitHashes,commitIndex + 1).then(newOwners => {
...
// };
//
// const transformResult = (result: {def: Declaration; owners: Owner[]}): {def: Declaration; owners: Owner[]} => ({
//     ...result,
//     owners: squish(
//         result.owners
